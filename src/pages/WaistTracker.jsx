...
export default function WaistTracker() {
  const { entries, addEntry, deleteEntry, loading } = useDailyEntries('waist_entries')
  const { entries: weightEntries } = useDailyEntries('weight_entries')
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true)
    try {
      await addEntry({ entry_date: date, waist_cm: Number(waist), notes: notes || null })
      setWaist('')
      setNotes('')
    } finally {
      setSaving(false)
  ...
  return (
    <div className="p-4 space-y-6 pb-24 max-w-2xl mx-auto">
      <header>
        <h1 className="text-xl font-semibold">Waist</h1>
        <p className="text-text-dim text-sm">Measure around the navel, under consistent conditions.</p>
      </header>
      ...
      <div className="bg-surface border border-border rounded-xl p-4">
        <WaistChart waistEntries={entries} weightEntries={weightEntries} />
      </div>
            ...
            {sortedDesc.map((entry) => (
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  <span className="text-text font-medium">{entry.waist_cm} cm</span>
                  <button
